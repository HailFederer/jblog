	...
	<c:import url="/WEB-INF/views/includes/header-blog.jsp"/>
		<div id="wrapper">
			<div id="content">
				...
				<c:forEach items="${postList}" var="postVo">
					<li><a href="${pageContext.request.contextPath}/${id}/${postVo.category_no}/${postVo.no}">
					${postVo.title }</a> <span>${postVo.reg_date }</span></li>
				</c:forEach>
				</ul>
			</div>
		</div>
		...
		<div id="navigation">
			...
			<c:forEach items="${categoryList}" var="categoryVo">
				<li><a href="${pageContext.request.contextPath}/${id}/${categoryVo.no }">
				${categoryVo.name }</a></li>
			</c:forEach>
			</ul>
		</div>
